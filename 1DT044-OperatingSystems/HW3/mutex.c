...
inc_atomic(void *arg __attribute__((unused)))
{
    ...
    for (i = 0; i < INC_ITERATIONS; i++) {
        __sync_fetch_and_add(&counter, INCREMENT);      //counter += DECREMENT; // You need to replace this
    }
...
dec_atomic(void *arg __attribute__((unused)))
{
    ...
    for (i = 0; i < DEC_ITERATIONS; i++) {
       __sync_fetch_and_sub(&counter, DECREMENT);     //counter += DECREMENT; // You need to replace this
    }
